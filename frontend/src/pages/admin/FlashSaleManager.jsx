...
const FlashSaleManager = () => {
  ...
  const handleSave = async (p) => {
    if (p.isFlashSale) {
      if (!p.flashSaleStartDate || !p.flashSaleEndTime)
        return alert("Thiếu ngày Bắt đầu/Kết thúc!");
      if (new Date(p.flashSaleStartDate) >= new Date(p.flashSaleEndTime))
        return alert("Ngày kết thúc phải sau ngày bắt đầu!");
      if (Number(p.soldCount) > Number(p.totalFlashSale))
        return alert("Số lượng 'Đã bán' ảo không thể lớn hơn Tổng suất!");
      if (Number(p.totalFlashSale) > Number(p.stock))
        return alert(
          `Lỗi: Tổng suất (${p.totalFlashSale}) > Tồn kho (${p.stock})!`,
        );
    }
    ...
    <div className="p-6 bg-gray-50 min-h-screen font-sans text-gray-700">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2 text-gray-800">
          <Zap className="text-yellow-500 fill-yellow-500" size={24} /> Quản Lý
          Flash Sale
        </h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
        {/* Thanh cuộn mảnh để Admin dễ nhìn thấy và thao tác */}
          ...
          <table className="w-full text-left border-separate border-spacing-0 min-w-[1400px]">
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {currentItems.map((p) => {
                ...
                const hasError =
                  isActive && Number(p.soldCount) > Number(p.totalFlashSale);

                return (
                  <tr
                    key={p._id}
                    className={`hover:bg-gray-50 transition ${isActive ? "bg-orange-50/30" : ""}`}
                  >
                    ...
                    <td className="p-3 text-center">
                      <button
                        onClick={() =>
                          handleChange(p._id, "isFlashSale", !p.isFlashSale)
                        }
                    ...
                    <td className="p-3">
                      <input
                        type="number"
                        min="0"
                        className="w-full p-2 border rounded text-red-600 font-bold focus:ring-1 focus:ring-red-500 text-sm"
                        value={p.flashSalePrice || 0}
                        onChange={(e) =>
                          handleChange(p._id, "flashSalePrice", e.target.value)
                        }
                        disabled={!isActive}
                      />
                    </td>
                    ...
                    <td className="p-3">
                      <input
                        type="number"
                        min="0"
                        className="w-full p-2 border rounded text-center bg-white text-sm"
                        value={p.totalFlashSale || 0}
                        onChange={(e) =>
                          handleChange(p._id, "totalFlashSale", e.target.value)
                        }
                        disabled={!isActive}
                      />
                    </td>
                    ...
                    <td className="p-3">
                      <input
                        ...
                        value={formatForInput(p.flashSaleStartDate)}
                        onChange={(e) =>
                          handleChange(
                            p._id,
                            "flashSaleStartDate",
                    ...
                    <td className="p-3">
                      <input
                        type="datetime-local"
                        className="w-full p-1.5 border rounded text-xs text-gray-600 focus:border-red-500 outline-none"
                        value={formatForInput(p.flashSaleEndTime)}
                        onChange={(e) =>
                          handleChange(
                            p._id,
                            "flashSaleEndTime",
                    ...
                    <td className="p-3 text-right">
                      <button
                        ...
                        className={`p-2 rounded-lg shadow-sm transition-all text-white flex items-center justify-center w-full h-9
