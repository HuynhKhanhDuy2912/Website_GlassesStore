...
const ContactPage = () => {
    ...
    const handleSelectTicket = async (ticket) => {
        ...
        if (ticket.isReadByUser === false) {
            try {
                ...
                setContacts(prev => prev.map(c => c._id === ticket._id ? { ...c, isReadByUser: true } : c));
            } catch (error) {
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            const res = await axios.put(`http://localhost:5000/api/contacts/${selectedTicket._id}/chat`, formData, { 
