...
const OrderDetailPage = () => {
  ...
  const statusConfig = {
    ...
    confirmed: { label: "ĐÃ XÁC NHẬN", color: "bg-orange-100 text-orange-800" },
        ...
        <div className="bg-blue-50/50 p-6 border-b border-blue-100 flex flex-wrap justify-between items-center gap-4">
          <div>
            <h1 className="text-xl font-bold text-gray-800 flex items-center gap-2">
              <Package className="text-blue-600" /> Đơn hàng #
          ...
          <div
            className={`px-4 py-2 rounded-lg font-bold text-xs uppercase tracking-widest ${currentStatus.color}`}
          >
            {currentStatus.label}
          </div>
        </div>
        ...
        <div className="p-6 grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-100">
              <h3 className="font-bold text-gray-800 mb-5 flex items-center gap-2 text-sm uppercase tracking-wider">
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
              <h3 className="font-bold text-gray-800 mb-5 flex items-center gap-2 text-sm uppercase tracking-wider">
                <CreditCard size={18} className="text-blue-600" /> Thông tin
                thanh toán
              </h3>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center text-sm border-t pt-4">
                  ...
                  {order.isPaid === "completed" ? (
                    <span className="flex items-center gap-1.5 font-bold text-emerald-600 bg-emerald-50 px-3 py-1.5 rounded-full shadow-sm shadow-emerald-100">
                      <CheckCircle size={14} /> Đã thanh toán
                    </span>
                  ) : (
                    <span className="flex items-center gap-1.5 font-bold text-orange-600 bg-orange-50 px-3 py-1.5 rounded-full shadow-sm shadow-orange-100">
            ...
            <div className="space-y-4">
              {order.items?.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex gap-4 p-4 bg-white border border-gray-100 rounded-xl items-center shadow-sm hover:shadow-md transition-shadow"
                  >
                    {/* Click vào ảnh để xem chi tiết */}
                    <Link
                      to={productLink}
                      className="w-20 h-20 rounded-lg overflow-hidden border border-gray-100 flex-shrink-0 bg-gray-50 block"
                    >
                      <img
                        src={
                    ...
                    <div className="flex-grow">
                      ...
                      <Link
                        to={productLink}
                        className="font-bold text-gray-800 hover:text-blue-600 transition-colors line-clamp-1"
                      >
                        {item.name}
                      </Link>
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        disabled={
                          processingItem === (item.product?._id || item.product)
                        }
